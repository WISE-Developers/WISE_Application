...
bool Project::WeatherStream::IsDateImportedFromFile(const WTime &time)
{
	return SUCCEEDED(m_stream->IsImportedFromFile(time));
}


HRESULT Project::WeatherStream::SetTempAlpha(double alpha)
{
	PolymorphicAttribute v = alpha;
	return m_stream->SetAttribute(CWFGM_WEATHER_OPTION_TEMP_ALPHA, v);
}


HRESULT Project::WeatherStream::SetTempBeta(double beta)
{
	PolymorphicAttribute v = beta;
	return m_stream->SetAttribute(CWFGM_WEATHER_OPTION_TEMP_BETA, v);
}


HRESULT Project::WeatherStream::SetTempGamma(double gamma)
{
	PolymorphicAttribute v = gamma;
	return m_stream->SetAttribute(CWFGM_WEATHER_OPTION_TEMP_GAMMA, v);
}


HRESULT Project::WeatherStream::SetWindAlpha(double alpha)
{
	PolymorphicAttribute v = alpha;
	return m_stream->SetAttribute(CWFGM_WEATHER_OPTION_WIND_ALPHA, v);
}


HRESULT Project::WeatherStream::SetWindBeta(double beta)
{
	PolymorphicAttribute v = beta;
	return m_stream->SetAttribute(CWFGM_WEATHER_OPTION_WIND_BETA, v);
}


HRESULT Project::WeatherStream::SetWindGamma(double gamma)
{
	PolymorphicAttribute v = gamma;
	return m_stream->SetAttribute(CWFGM_WEATHER_OPTION_WIND_GAMMA, v);
}


HRESULT Project::WeatherStream::SetFFMCMethod(short method)
{
	PolymorphicAttribute v = true;
	switch (method) {
		case 0:		return m_stream->SetAttribute(CWFGM_WEATHER_OPTION_FFMC_VANWAGNER, v);
		case 2:		return m_stream->SetAttribute(CWFGM_WEATHER_OPTION_FFMC_LAWSON, v);
	}
	return E_INVALIDARG;
}
