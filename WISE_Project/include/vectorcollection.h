...
namespace Project
{
	...
	public:
		...
		Vector *LN_Succ() const					{ return (Vector *)Node::LN_Succ(); }
		Vector *LN_Pred() const					{ return (Vector *)Node::LN_Pred(); }

		virtual void PostSerialize(class CWFGMProject* /*project*/) { }

		const TCHAR *Name(const TCHAR *name);
		const TCHAR *Name() const					{ return ln_Name; }
		...
		COLORREF GetFillColor() const				{ if (m_fillColor.has_value()) return m_fillColor.value(); return RGB(0, 0, 0); }
		...
		HRESULT LockState()						{ if (m_filter) return m_filter->MT_Lock(false, (USHORT)-1); return S_OK; }
...
namespace Project
{
	...
	public:
		...
		ULONG AddPolyLine(const XY_PolyConst &poly, bool is_polygon)		{ return AddPolyLine(poly.GetPointsArray(), poly.NumPoints(), is_polygon); };
		ULONG SetPolyline(ULONG index, const XY_PolyConst &poly, bool is_polygon)	{ return SetPolyLine(index, poly.GetPointsArray(), poly.NumPoints(), is_polygon); };
...
namespace Project
{
	...
	public:
		...
		void AddVector(Vector *vector)				{ m_vectorList.AddTail(vector); };
		void InsertVector(Vector *vector, ULONG index)	{ m_vectorList.InsertIndex(vector, index); };
		void RemoveVector(Vector *vector)			{ m_vectorList.Remove(vector); };
		ULONG GetCount() const						{ return m_vectorList.GetCount(); };
		Vector *FirstVector() const				{ return (Vector *)m_vectorList.LH_Head(); };
		Vector *VectorAtIndex(ULONG index) const	{ return (Vector *)m_vectorList.IndexNode(index); };
		ULONG IndexOf(Vector *vector) const		{ return m_vectorList.NodeIndex(vector); };
		Vector *FindName(const TCHAR *name) const	{ return (Vector *)m_vectorList.FindName(name); };
